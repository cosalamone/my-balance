...
export class IncomeUtils {
  ...
  static getCategoryColor(category: IncomeCategory): string {
  ...
  static getCategoryBadgeClass(category: IncomeCategory): string {
  ...
  static calculateMonthlyAverage(incomes: any[], months: number = 12): number {
  ...
  static validateAmount(amount: number): { isValid: boolean; message?: string } {
    if (isNaN(amount) || amount <= 0) {
      return { isValid: false, message: 'El monto debe ser un número mayor a 0' };
    }
  ...
  static validateDescription(description: string): { isValid: boolean; message?: string } {
    ...
    if (description.length > 100) {
      return { isValid: false, message: 'La descripción no puede exceder 100 caracteres' };
    }
  ...
  static generateIncomeId(): string {
    return 'income_' + Math.random().toString(36).substr(2, 9) + '_' + Date.now();
  }

  static sortIncomesByDate(incomes: any[], ascending: boolean = false): any[] {
  ...
  static groupIncomesByCategory(incomes: any[]): Record<string, any[]> {
  ...
  static downloadCSV(csvContent: string, filename: string = 'ingresos.csv'): void {
